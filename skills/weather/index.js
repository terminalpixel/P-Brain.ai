...
class WeatherSkill extends PBrainSkill {
    constructor() {
        super('weather');
    }

    keywords() {
        return ['what is the weather', 'whats it like in qqqq', 'temperature', 'whats the weather qqqq']
    }

    examples() {
        return ['What\'s the weather like?', 'What\'s it like in France?', 'What\'s the temperature?']
    }
    ...
    * get_country() {
    ...
    get_new_day_index(forcasted_day) {
        let new_index
    ...
    * get(query) {
        ...
        let forcast = false
        let current_loc = yield this.get_country()
        ...
        if (query.indexOf(' in ') != -1) {
            ...
            if (country.indexOf(' tomorrow') != -1) {
                index = this.get_index_from_day() + 1
            } else {
                index = this.get_index_from_day(country)
            }
